...
def get_images(request):
    ...
    shutter = request.GET.get('shutter')
    if shutter and shutter != 'all':
        images = images.filter(shutter=shutter)

    channel = request.GET.get('channel')
    if channel and channel != 'all':
        images = images.filter(channel=channel)
...
def images_list(request):
    ...
    shutters = images.distinct('shutter').values('shutter')
    context['shutters'] = shutters

    channels = images.distinct('channel').values('channel')
